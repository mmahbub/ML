...
I = find(test(:,3) == 0);  % find the row indices for the 1st class, labeled as 0
test0 = test(I,1:2);
save train0;                % so that you can use it directly next time 

I = find(test(:,3) == 1);  % find the row indices for the 2nd class, labeled as 1
test1 = test(I,1:2);
save train1;

% plot the testing dataset
plot(test0(:,1), test0(:,2), 'r*', test1(:,1), test1(:,2), 'go')
hold on
...
px=px0-px1
plt1 = fimplicit(px0-px1, [-1.5 1.5 -1.5 1.5])
...
px1 = (1/((2*pi)^(d/2)*sqrt(detercov1)))*exp(-.5*transpose(feat - mean1)*inversecov1*(feat - mean1))*prob1;
px=px0-px1
plt2 = fimplicit(px0-px1, [-1.5 1.5 -1.5 1.5])
...
px1 = (1/((2*pi)^(d/2)*sqrt(detercov1)))*exp(-.5*transpose(feat - mean1)*inversecov1*(feat - mean1))*prob1;
px=px0-px1
plt3 = fimplicit(px0-px1, [-1.5 1.5 -1.5 1.5])
